...
\lstinputlisting[linerange=29-41,numbers=none]{TP_proj1.py}
...
\lstinputlisting[linerange=43-53,numbers=none]{TP_proj1.py}
...
\lstinputlisting[linerange=68-75,numbers=none]{TP_proj1.py}

Where \texttt{first} and \texttt{second} refers to the first and second axis of the phase-space (in this case x-px) . Both are a 3-component lists of the kind \texttt{[min,max,num]} giving a \texttt
{num} equal binning between \texttt{min} and \texttt{max}.

This will allow you to visualise the electrostatic field energy averaged over space as a function of time,  the electric field and the electron density  as a function of space and the momentum and position of the particles in the code (phase space) by clicking on \texttt{plot}.

Notice that we are exciting a stationary wave. \\
{\bf (2.5)} Compute analytically the time evolution of the total electrostatic field energy:
