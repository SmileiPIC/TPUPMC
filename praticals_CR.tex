...
% Title
%-------
\title{TP - UPMC \\ Introduction to plasma physics with Particle-In-Cell simulations\footnote{These notes are available at: \url{https://github.com/iltommi/TPUPMC}}}


\author{Caterina~Riconda \thanks{\url{caterina.riconda@upmc.fr}}, \and Tommaso Vinci \thanks{\url{tommaso.vinci@polytechnique.edu}}, \and Mickael~Grech \thanks{\url{mickael.grech@polytechnique.edu}} }
...
We first give some details on the open-source PIC code SMILEI\footnote{homepage: \url{http://www.maisondelasimulation.fr/smilei}}\footnote{download: \url{https://github.com/SmileiPIC/Smilei}} currently developed at LULI in collaboration with various laboratories.
...
\vspace{0.5cm}
  \Exercise[title={Simulations of electron-plasma oscillations},label={ex2}] \leavevmode
    ...
    \Question{In the limit of small perturbations (i.e.\ $\delta n \ll 1$ in Eq.~\ref{eq_pert}), derive analytically the equation for $n_e(t,x)$ for the case of a cold plasma with immobile ions.   What is the theoretical frequency of plasma oscillations in normalised units? Write the solution in the form of a stationary wave with an arbitrary phase}
    \Question{Derive the relationship between $n_e(t,x)$, $u_{e,x}(t,x)$ and $E_x(t,x)$ from the fluid equation system including Maxwell's equations.}
    ...
    Run the PIC simulation with input file \texttt{TP\_proj1.py}. To do so write in the \texttt{smilei} directory the command 
...
Notice that we are exciting a stationary wave. 
    ...
    \Question{From the simulations, deduce the explicit form of $n_{1,e}(x,t)$ (i.e. by considering equation (8) and the simulation results deduce the actual value of the arbitrary  phase introduced in section (2.1)) and discuss the wave evolution in time. What is the characteristic time for the electric field oscillations (in normalised as well as physical units)? What about density oscillations? What is the relative phase between the electric field and density oscillations?  Are results consistent with theoretical predictions from exercise (\ref{ex2}.1) and (\ref{ex2}.2)?}
...
\vspace{0.5cm}
  \Exercise[title={Effect of the temporal resolution},label={ex3}] \leavevmode
    ...
    \Question{Verify that you are back to 10 particles per cell. For the temporal resolution $\Delta t = 0.2$ calculate the correction to the frequency from the formula above.  
    
    Take in the code \code{timestep} = 0.2 , \code{cell\_length} = 0.24, \code{sim\_lenght}= 24., and in the \class{DiagPhase} take the second value of  the first component of \code{axes} = 24. (maximum value for the position/plasma length), and for the the second component of \code{axes} \texttt{["px", -0.12, 0.12,100]} (extremes values for the momentum).   Deduce the oscillation frequency form the time evolution of the electrostatic energy in the code and compare with the theoretical result. Comment the result.}
...
\vspace{0.5cm}
 ...
 in the \class{DiagPhase} take the second value of  the first component of \code{axes} = 1. (maximum value for the position/plasma length), and for the the second component of \code{axes} : \texttt{["px", -0.02, 0.02, 100]} (extremes values for the momentum). 
...
%\includegraphics[width=\linewidth]{Fig_2stream.pdf}
...
\begin{ExerciseList} 
\vspace{0.5cm}
  \Exercise[title={Dispersion relation, characteristic frequencies and growth rate: Theory},label={ex6}] \leavevmode
...
\vspace{0.5cm}
  \Exercise[title={Simulation for $v_0 k > 1$},label={ex7}] \leavevmode
    ...
    \Question{Run the corresponding simulations.  For the phase-space diagnostics, in the input file \texttt{TP\_proj2.py}, take for the extremes values of the momentum in \class{DiagPhase} $\mp 0.11$ respectively. For simulations with \code{sim\_length} $= 0.62$ and $0.60$, extract from the field energy temporal evolution the characteristic frequencies (try to estimate error bars for your measurements). Report them on the analytical predictions (from Exercise \ref{ex6}). Why does it become more difficult to extract the characteristic frequencies for \code{sim\_length}~$= 0.31 - 0.16$ ?}
    \Question{ Discuss the general behaviour of [\rom{1}] the energy in the electrostatic field, and [\rom{2}] the behaviour in phase-space of the electron species. }
\end{ExerciseList}
...
\vspace{0.5cm}
  \Exercise[title={Simulation for $v_0 k < 1$},label={ex8}] \leavevmode
    ...
    \Question{Compute the values of $v_0 k$ corresponding to \code{sim\_length} $= 0.69 - 1.03 - 1.68$. Calculate the corresponding values of $\omega$ and $\gamma$ from Eq. \ref{eqomega}.\\Set the density perturbation $\delta n = 0.001$.}
    \Question{Run the corresponding simulations. For the phase-space diagnostics, in the input file \texttt{TP\_proj2.py},  take for the extremes values of the momentum in \class{DiagPhase} section (i.e.\ the  \texttt{"px"}  part of the \code{axes})   to $\pm 0.4$, respectively. For each simulation (i.e. for different value of the parameter $v_0 k$), extract from the field energy temporal evolution the characteristic frequency and growth rate of the instability (it is convenient to consider the energy plot in log-scale. To do so, just click on the corresponding panel and letter `l'). Report them on the analytical predictions (from Exercise \ref{ex6}).}
    \Question{For each simulation, discuss the general behaviour of (i) the energy in the electrostatic field, and (ii) the behaviour in phase-space of the electron species.}
